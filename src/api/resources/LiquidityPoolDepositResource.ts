...
export class LiquidityPoolDepositResource extends BaseEntityResource {
  ...
  toJson(entity: LiquidityPoolDeposit): Object {
    const response: any = {
      depositAToken: entity.depositAToken
        ? this._assetResource.toJson(entity.depositAToken)
        : null,
      depositBToken: entity.depositBToken
        ? this._assetResource.toJson(entity.depositBToken)
        : null,
    ...
    if (entity.statuses?.length > 0) {
      response.statuses = entity.statuses.map((status: OperationStatus) =>
        this._operationResource.toJson(status)
      );
    }
  ...
  toCompressed(entity: LiquidityPoolDeposit): Object {
    const response: any = {
      ...
      dAT: entity.depositAToken
        ? this._assetResource.toCompressed(entity.depositAToken)
        : null,
      dBT: entity.depositBToken
        ? this._assetResource.toCompressed(entity.depositBToken)
        : null,
    ...
    if (entity.statuses?.length > 0) {
      response.st = entity.statuses.map((status: OperationStatus) =>
        this._operationResource.toCompressed(status)
      );
    }

...
export class LiquidityPoolController extends BaseApiController {
  public bootRoutes(): void {
    ...
    this.router.post(
      `${this.basePath}/deposits/historic`,
      this.depositsHistoric
    );
    this.router.post(
      `${this.basePath}/withdraws/historic`,
      this.withdrawsHistoric
    );
    this.router.post(
      `${this.basePath}/states/historic`,
      this.liquidityPoolStatesHistoric
    );

    this.router.get(`${this.basePath}/:identifier`, this.liquidityPool);
    this.router.get(
      `${this.basePath}/:identifier/ticks`,
      this.liquidityPoolTicks
    );
    this.router.get(
      `${this.basePath}/:identifier/swaps`,
      this.liquidityPoolSwaps
    );
    this.router.get(
      `${this.basePath}/:identifier/deposits`,
      this.liquidityPoolDeposits
    );
    this.router.get(
      `${this.basePath}/:identifier/withdraws`,
      this.liquidityPoolWithdraws
    );
  }

  private swapsHistoric(request: express.Request, response: express.Response) {
    const { fromTimestamp, toTimestamp, forAssets } = request.body;

    if (forAssets && !(forAssets instanceof Array)) {
      return response.send(super.failResponse('Assets must be an array'));
    }

    const assets: Asset[] = ((forAssets ?? []) as string[]).map(
      (identifier: string) => Asset.fromId(identifier)
    );
      ...
      return manager
        ...
        .andWhere(
          new Brackets((query) => {
            if (assets.length > 0) {
              query.andWhere(
                '(swapInToken.policyId IN(:policyIds) AND swapInToken.nameHex IN(:nameHexs)) OR (swapOutToken.policyId IN(:policyIds) AND swapOutToken.nameHex IN(:nameHexs))',
                {
                  policyIds,
                  nameHexs,
                }
  ...
  private liquidityPoolStatesHistoric(
    ...
    const fetchStates: any = (manager: EntityManager) => {
      return manager
        .createQueryBuilder(LiquidityPoolState, 'states')
        ...
        .andWhere(
          new Brackets((query) => {
            if (assets.length > 0) {
              query.andWhere(
                '(tokenA.policyId IN(:policyIds) AND tokenA.nameHex IN(:nameHexs)) OR (tokenB.policyId IN(:policyIds) AND tokenB.nameHex IN(:nameHexs))',
                {
                  policyIds,
                  nameHexs,
                }
    ...
    return dbApiService
      .query(fetchStates)
      .then((states) => {
        const resource: LiquidityPoolStateResource =
          new LiquidityPoolStateResource();
      ...
      return manager
        ...
        .andWhere(
          new Brackets((query) => {
            if (assets.length > 0) {
              query.andWhere(
                '(depositAToken.policyId IN(:policyIds) AND depositAToken.nameHex IN(:nameHexs)) OR (depositBToken.policyId IN(:policyIds) AND depositBToken.nameHex IN(:nameHexs))',
                {
                  policyIds,
                  nameHexs,
                }
  ...
  private withdrawsHistoric(
    ...
    const fetchOrders: any = (manager: EntityManager) => {
      return manager
        ...
        .addSelect(['lpToken.policyId', 'lpToken.nameHex', 'lpToken.isLpToken'])
        .leftJoinAndMapMany(
          'withdraws.statuses',
          OperationStatus,
          'status',
  ...
  private liquidityPools(request: express.Request, response: express.Response) {
    const { identifier, dex, tokenA, tokenB, fromTimestamp, toTimestamp } =
      request.body;
            ...
            new Brackets((query) => {
              ...
              if (fromTimestamp) {
                query.andWhere('pools.createdSlot >= :fromSlot', {
                  fromSlot: unixTimeToSlot(
                    'Mainnet',
                    Number(fromTimestamp) * 1000
                  ),
                });
              }
  ...
  private liquidityPool(request: express.Request, response: express.Response) {
    ...
    return dbApiService
      ...
      .catch(() =>
        response.send(super.failResponse('Unable to retrieve liquidity pool'))
      );
  }

  private liquidityPoolTicks(
    request: express.Request,
    response: express.Response
  ) {
    const { identifier } = request.params;
    const { resolution, fromTime, toTime, orderBy } = request.query;
    ...
    if (orderBy && !['ASC', 'DESC'].includes(orderBy as string)) {
      return response.send(
        super.failResponse("orderBy must be 'ASC' or 'DESC'")
      );
    }
        ...
        .then((pool: LiquidityPool | null) => {
          ...
          return manager
            ...
            .where(
              new Brackets((query) => {
                query
                  .where('ticks.liquidityPoolId = :poolId', {
                    poolId: pool.id,
                  })
    ...
    return dbApiService
      ...
      .then((ticks: LiquidityPoolTick[]) => {
        const resource: LiquidityPoolTickResource =
          new LiquidityPoolTickResource();
  ...
  private async liquidityPoolSwaps(
    request: express.Request,
    response: express.Response
  ) {
    const { identifier } = request.params;
    const { type, page, limit, sender } = request.query;
    ...
    const liquidityPool: LiquidityPool | null = await dbApiService.query(
          ...
          new Brackets((query) => {
            if (sender) {
              if ((sender as string).startsWith('addr')) {
                query
                  .where('swaps.senderPubKeyHash = :hash', {
            ...
            if (type && type === 'buy') {
              if (!liquidityPool.tokenA) {
                ...
                query.andWhere('swaps.swapInTokenId = :tokenAId', {
                  tokenAId: liquidityPool.tokenA?.id ?? 0,
                });
              }
            }
            if (type && type === 'sell') {
              if (!liquidityPool.tokenA) {
                ...
                query.andWhere('swaps.swapOutTokenId = :tokenAId', {
                  tokenAId: liquidityPool.tokenA?.id ?? 0,
                });
              }
            }
    ...
    return dbApiService
      ...
      .then(([swaps, total]) => {
        ...
        swaps.forEach(
          (order: LiquidityPoolSwap) => (order.liquidityPool = liquidityPool)
        );
  ...
  private async liquidityPoolDeposits(
    request: express.Request,
    response: express.Response
  ) {
    const { identifier } = request.params;
    const { page, limit, sender } = request.query;
        ...
        .andWhere(
          new Brackets((query) => {
            if (sender) {
              if ((sender as string).startsWith('addr')) {
                query
                  .where('deposits.senderPubKeyHash = :hash', {
                    hash: paymentCredentialOf(sender as string).hash,
                  })
  ...
  private async liquidityPoolWithdraws(
    request: express.Request,
    response: express.Response
  ) {
    const { identifier } = request.params;
        ...
        .andWhere(
          new Brackets((query) => {
            if (sender) {
              if ((sender as string).startsWith('addr')) {
                query
                  .where('withdraws.senderPubKeyHash = :hash', {
                    hash: paymentCredentialOf(sender as string).hash,
                  })
  ...
  private search(request: express.Request, response: express.Response) {
    ...
    const searchQuery: string =
      '%' + (query as string).replace(/[^a-z0-9\s]/gi, '').toLowerCase() + '%';
    ...
    return dbApiService
      .query((manager: EntityManager) => {
        return manager
          ...
          .andWhere(
            new Brackets((query) => {
              query
                .where('LOWER(tokenA.name) LIKE :query', {
                  query: searchQuery,
                })
      ...
      .catch(() =>
        response.send(super.failResponse('Unable to search liquidity pools'))
      );
  }

  private liquidityPoolPrices(
    request: express.Request,
    response: express.Response
  ) {
    ...
    return dbApiService
      ...
      .then((results: any) => {
        response.send(
          results.reduce((prices: Object[], entry: any) => {
            ...
            const tokenADecimals: number = entry.tokenA
              ? entry.tokenA.decimals
              : 6;
            const tokenBDecimals: number = entry.tokenB.decimals ?? 0;

            const price: number =
              entry.latestState.reserveA /
              10 ** tokenADecimals /
              (entry.latestState.reserveB / 10 ** tokenBDecimals);
            ...
            prices.push({
              ...
              hourChange: !entry.hour_tick
                ? 0
                : ((price - entry.hour_tick.open) / entry.hour_tick.open) * 100,
            });
      ...
      .catch(() =>
        response.send(
          super.failResponse('Unable to retrieve liquidity pool prices')
        )
      );
  }
}

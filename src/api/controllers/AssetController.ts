...
export class AssetController extends BaseApiController {
  ...
  private assets(request: express.Request, response: express.Response) {
    const { policyId, nameHex, isVerified } = request.body;
    ...
    dbApiService
      ...
      .catch(() =>
        response.send(super.failResponse('Unable to retrieve assets'))
      );
  }

  private search(request: express.Request, response: express.Response) {
    const { query, onlyVerified, page, limit } = request.query;
    ...
    dbApiService
        ...
        return manager
          ...
          .andWhere(
            new Brackets((query) => {
              ...
              query
                .where('LOWER(asset.name) LIKE :query', {
                  query: searchQuery,
                })
                ...
                .orWhere(
                  'CAST(unhex(asset.nameHex) AS CHAR(100)) LIKE :query',
                  {
                    query: searchQuery,
                  }
                );
      ...
      .catch(() =>
        response.send(super.failResponse('Unable to search assets'))
      );
  }

  private lpTokenPool(request: express.Request, response: express.Response) {
    const { lpToken } = request.params;
    ...
    return dbApiService
          ...
          .then((state: LiquidityPoolState | null) => {
            if (!state || !state.liquidityPool) {
              return response.send(
                super.failResponse('Token not assigned to a pool')
              );
            }
      ...
      .catch(() =>
        response.send(super.failResponse('Token not assigned to a pool'))
      );
  }
    ...
    return dbApiService
      .query((manager: EntityManager) => {
        return manager
          ...
          .andWhere(
            'tokenB.policyId = :policyId AND tokenB.nameHex = :nameHex',
            {
              policyId: tokenPolicyId,
              nameHex: tokenNameHex,
            }
          )
          .andWhere('latestState.tvl >= :minTvl', {
            minTvl: 100_000_000000,
          })
          .getMany();
      })
      .then((liquidityPools: LiquidityPool[]) => {
        ...
        const avgPrice: number =
          liquidityPools.reduce((avgPrice: number, pool: LiquidityPool) => {
            const tokenADecimals: number = 6;
            const tokenBDecimals: number = pool.tokenB.decimals ?? 0;
            const price: number =
              +pool.latestState.reserveA /
              10 ** tokenADecimals /
              (+pool.latestState.reserveB / 10 ** tokenBDecimals);
      ...
      .catch(() =>
        response.send(super.failResponse('Unable to retrieve asset price'))
      );
  }

  private ticks(request: express.Request, response: express.Response) {
    const { forAssets } = request.body;
    ...
    const fetchTicks: any = (manager: EntityManager) => {
      return manager
        ...
        .andWhere(
          new Brackets((query) => {
            query.andWhere(
              'tokenB.policyId IN(:policyIds) AND tokenB.nameHex IN(:nameHexs)',
              {
                policyIds,
                nameHexs,
              }

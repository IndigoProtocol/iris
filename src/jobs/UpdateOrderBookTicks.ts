...
export class UpdateOrderBookTicks extends BaseJob {
  ...
  public async handle(): Promise<any> {
    ...
    const slotDate: Date = new Date(
      slotToUnixTime('Mainnet', this._match.slot)
    );
  ...
  private async createOrUpdateTick(
    ...
    const price: number = tokensMatch(
      this._match.orderBook.tokenA ?? 'lovelace',
      this._match.matchedToken ?? 'lovelace'
    )
      ? 1 / this._match.referenceOrder.price
      : this._match.referenceOrder.price;

    const existingTick: OrderBookTick | undefined = await dbService.query(
    ...
    if (!existingTick) {
      const lastTick: OrderBookTick | undefined = await dbService.query(
        (manager: EntityManager) => {
          return (
            manager
              .createQueryBuilder(OrderBookTick, 'ticks')
    ...
    return dbService.transaction((manager: EntityManager) => {
      return manager.save(existingTick).then((tick: OrderBookTick) => {
        operationWs.broadcast(tick);

...
export class UpdateAmountReceived extends BaseJob {
  ...
  public async handle(): Promise<any> {
    if (!this._liquidityPoolState.liquidityPool)
      return Promise.reject('Liquidity pool not provided');

    logInfo(
      `[Queue] \t UpdateAmountReceived for state ${this._liquidityPoolState.txHash}`
    );

    let swapOrders: LiquidityPoolSwap[] | null = await dbService.dbSource
      .createQueryBuilder(LiquidityPoolSwap, 'orders')
      .leftJoinAndSelect('orders.swapOutToken', 'swapOutToken')
      .where('orders.txHash IN(:...txHashes)', {
        txHashes: this._liquidityPoolState.transactionInputs.map(
          (input: Utxo) => input.forTxHash
        ),
      })
      .andWhere('orders.outputIndex IN(:...indexes)', {
        indexes: this._liquidityPoolState.transactionInputs.map(
          (input: Utxo) => input.index
        ),
      })
      .andWhere('orders.liquidityPoolId = :poolId', {
        poolId: this._liquidityPoolState.liquidityPool.id,
      })
      .getMany();
    ...
    if (!swapOrders)
      return Promise.reject(
        'Unable to find linked swap order to update amount received'
      );

    swapOrders = swapOrders
      .map((swapOrder: LiquidityPoolSwap) => {
        const settledUtxo: Utxo | undefined =
          this._liquidityPoolState.transactionOutputs.find((utxo: Utxo) => {
        ...
        const amountToAddress: bigint = !swapOrder.swapOutToken
          ...
          : settledUtxo.assetBalances.find(
              (assetBalance: AssetBalance) =>
                tokenId(assetBalance.asset) ===
                tokenId(swapOrder.swapOutToken as Asset)
            )?.quantity ?? 0n;
        ...
        logInfo(
          `[Queue] \t\t Updating receive for swap order ${swapOrder.txHash}`
        );

        return swapOrder;
      })
      .filter(
        (order: LiquidityPoolSwap | undefined) => order !== undefined

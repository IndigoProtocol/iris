...
export class ReserveSplitter extends BaseRouter {
    ...
    calculateRouting(pools: LiquidityPool[], swapAmount: bigint, swapInToken: Token, swapOutToken: Token, isReversed: boolean = false): OrderRouteResults {
        const totalReserveA: number = pools.reduce((totalReserveA: number, pool: LiquidityPool) => {
            return totalReserveA + pool.latestState.reserveA;
        }, 0);
        ...
        const splitResults: OrderRouteResult = pools.reduce((results: OrderRouteResult, pool: LiquidityPool) => {
            const splitPercent: number = Number(
                (pool.latestState.reserveA / totalReserveA).toFixed(3)
            );
            ...
            results[pool.dex] = {
                swapInAmount: Number(swapAmount) / 10**(swapInToken === 'lovelace' ? 6 : swapInToken.decimals),

...
export class TokenMetadataService extends BaseService {
  boot(): Promise<any> {
    if (!CONFIG.GITHUB_ACCESS_TOKEN)
      return Promise.reject('Github access token not set in env.');
  ...
  fetchAsset(policyId: string, nameHex: string): Promise<TokenMetadata> {
    return axios
      .get(
        `https://raw.githubusercontent.com/cardano-foundation/cardano-token-registry/master/mappings/${policyId}${nameHex}.json`,
        {
          headers: {

...
export class OrderBookOperationHandler {
      ...
      if ('dex' in operation) {
        logInfo(
          `[${operation.dex}] ${operation.constructor.name} ${
            (operation as OrderBookDexOperation).txHash
          }`
        );
      } else if (
        'type' in operation &&
  ...
  private async handleOperation(
    operation: OrderBookDexOperation
    ...
    if (
      ...
      return await this.handleCancellation(
        operation as OrderBookOrderCancellation
      );
    }
  ...
  private async handleCancellation(
    cancellation: OrderBookOrderCancellation
  ): Promise<OrderBookOrder> {
    const existingOrder: OrderBookOrder | undefined = await dbService.query(
      (manager: EntityManager) => {
        return (
          manager.findOneBy(OrderBookOrder, {
    ...
    if (!existingOrder) {
      return Promise.reject(
        `Unable to find cancelled order from ${cancellation.txHash}`
      );
    }
    ...
    return await dbService.transaction(
      async (manager: EntityManager): Promise<OrderBookOrder> => {
  ...
  private async handleOrder(order: OrderBookOrder): Promise<OrderBookOrder> {
    const existingOrderConditions: FindOptionsWhere<OrderBookOrder>[] = [
      {
        txHash: In(
          order.transaction?.inputs.map((input: Utxo) => input.forTxHash) ?? []
        ),
    ...
    const existingOrder: OrderBookOrder | undefined = await dbService.query(
      (manager: EntityManager) => {
        return (
          manager.findOneBy(OrderBookOrder, existingOrderConditions) ??
          undefined
        );
      }
    );
  ...
  private async handleMatch(match: OrderBookMatch): Promise<OrderBookMatch> {
    const existingOrderConditions: FindOptionsWhere<OrderBookOrder>[] = [
      {
        txHash: In(
          match.transaction?.inputs.map((input: Utxo) => input.forTxHash) ?? []
        ),
    ...
    const existingOrders: OrderBookOrder[] | undefined = await dbService.query(
      (manager: EntityManager) => {
        return (
          manager.find(OrderBookOrder, {
    ...
    if (!existingOrders || existingOrders.length === 0) {
      return Promise.reject(
        `Unable to find fromOrder from match in ${match.txHash}`
      );
    }
    ...
    if (match.consumedTxHash) {
      match.matchedAmount = existingOrders.reduce(
        (total: number, order: OrderBookOrder) =>
          total + order.unFilledOfferAmount,
        0
      );
      existingOrders.forEach(
        (order: OrderBookOrder) => (order.unFilledOfferAmount = 0)
      );
    } else if (match.unFilledAmount) {
      match.matchedAmount =
        existingOrders.reduce(
          (total: number, order: OrderBookOrder) => total + order.askedAmount,
          0
        ) - match.unFilledAmount;
      existingOrders.forEach(
        (order: OrderBookOrder) =>
          (order.unFilledOfferAmount = match.unFilledAmount)
      );
      ...
      match.matchedAmount =
        existingOrders.reduce(
          (total: number, order: OrderBookOrder) => total + order.askedAmount,
          0
        ) - match.referenceOrder.unFilledOfferAmount;
      existingOrders.forEach(
        (order: OrderBookOrder) =>
          (order.unFilledOfferAmount -= match.unFilledAmount)
      );
    ...
    existingOrders.forEach(
      (order: OrderBookOrder) => (order.numPartialFills += 1)
    );

    match.referenceOrder = existingOrders[0];

    return dbService.transaction(
      async (manager: EntityManager): Promise<OrderBookMatch> => {
        await manager.save(existingOrders);

        existingOrders.forEach((order: OrderBookOrder) =>
          eventService.pushEvent({
            type: 'OrderBookOrderUpdated',
            data: order,
          })
        ...
        return manager.save(match).then((match: OrderBookMatch) => {
  ...
  private async retrieveOrderBook(
    dex: Dex,
    tokenA: Token,
    tokenB: Token,
    slot: number
    ...
    const aToken: Token =
      tokenA === 'lovelace' ? tokenA : tokenB === 'lovelace' ? tokenB : tokenA;
    const bToken: Asset = tokensMatch(aToken, tokenA)
      ? (tokenB as Asset)
      : (tokenA as Asset);

    const existingBook: OrderBook | undefined = await dbService.query(
      async (manager: EntityManager) => {
        return (
          (await manager.findOne(OrderBook, {
    ...
    return await dbService.transaction(
      async (manager: EntityManager): Promise<OrderBook> => {
        ...
        return await manager.save(orderBook).then(() => {

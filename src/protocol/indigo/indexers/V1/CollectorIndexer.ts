 ...
 */
export class CollectorIndexer extends BaseV1Indexer {
    private collectorAddr = config.V1_COLLECTOR_ADDRESS;
    ...
    async processTransaction(tx: Transaction, slot: Slot) {
        ...
        const collectorOutputs = tx.outputs.filter(
            (out) => out.address === this.collectorAddr
        );
        if (collectorOutputs.length > 0) {
            ...
            const collectorOutputValue = collectorOutputs.reduce(
                (prev, current) => prev + BigInt(current.value.ada.lovelace),
                0n
            );

            await Promise.all(
                collectorOutputs.map((output: TransactionOutput, index: number) =>

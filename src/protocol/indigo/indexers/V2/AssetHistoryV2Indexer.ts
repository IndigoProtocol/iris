 ...
 */
export class AssetHistoryV2Indexer extends BaseV2Indexer {
    /*
        ...
        for (const key in tx.outputs) {
            ...
            if (
                ...
                const iassetData = {
                    ...
                    base_rates: stringify(iasset.baseRates),
                    version: 'v2'
                };
    ...
    private toIAssetFromDatum(datum: string): iAsset {
        ...
        return {
            ...
            redemptionRatioPercentage: iAssetDatum[2].value[0],
            maintenanceRatioPercentage: iAssetDatum[3].value[0],
            liquidationRatioPercentage: iAssetDatum[4].value[0],
            debtMintingFeePercentage: iAssetDatum[5].value[0],
            liquidationProcessingFeePercentage: iAssetDatum[6].value[0],
            stabilityPoolWithdrawalFeePercentage: iAssetDatum[7].value[0],
            redemptionReimbursementPercentage: iAssetDatum[8].value[0],
            redemptionProcessingFeePercentage: iAssetDatum[9].value[0],
            baseRates: iAssetDatum[10].map((x: any) => {
                return {
                    rate: x.value[0].value[0] / 10_000,
                    active_from: x.value[1],
                }
            }),
            // The indexer doesn't care about first/next asset
            // firstAsset: iAssetDatum[11].tag === 121,
            // nextAsset: iAssetDatum[11].tag === 121 ? iAssetDatum[11].value[0] : undefined,
        } as iAsset;
    }

...
import { CollateralizedDebtPositionUtxoV2Indexer } from './CollateralizedDebtPositionUtxoV2Indexer';

/**
 * This Indexer updates the database with redemption events.
 */
export class RedemptionV2Indexer extends BaseV2Indexer {
    /**
    ...
    async processTransaction(tx: Transaction, slot: Slot) {
        ...
        const collateralRedeemed = Math.floor(Number(collateralDifference) * (1 + (asset.redemption_reimbursement_percentage / 100_000_000)))
        ...
        const cdp = CollateralizedDebtPositionUtxoV2Indexer.toCDP(cdpOutput.datum);
        
        console.log('Redemption Found:', collateralRedeemed, assetRedeemed, cdpInput.asset, tx.id, slot, processingFee, reimbursementFee, cdp.owner);
